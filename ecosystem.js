...
function ecosystem(sim, nbRunners, nbPredators) {
    ...
    function getMousePos(evt) {
        ...
        return vec2.fromValues(
            (evt.clientX-rect.left)/(rect.right-rect.left)*sim.canvas.width,
            (evt.clientY-rect.top)/(rect.bottom-rect.top)*sim.canvas.height);
    }

    sim.canvas.onmousemove = (evt) => attractor.loc = getMousePos(evt);
